...
const FORM = document.querySelector('.bulb-form')
const TOGGLE = FORM.querySelector('.bulb-form button')
...
for (let i = 1; i < CORDS.length; i++) {
  const startShape = CORDS[0].getAttribute('d');
  const endShape = CORDS[i].getAttribute('d');

  console.log(endShape);

  CORD_TL.add(
    gsap.to(CORDS[0], {
      attr: { d: endShape },  // Manually animating the 'd' attribute of the path
